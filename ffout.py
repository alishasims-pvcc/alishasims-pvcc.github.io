...
############# output file ##############
    ...
    titles2 = "\nFed Inc Tax" + tab + "State Inc Tax" + tab + "Soc Sec" + tab + "Medicare" + tab + "Net"
    f.write(titles1 + titles2)

    for i in range(num_emps):
        data1 = "\n" + emp[1] + " " + job[i] + format(gross_pay[i], currency)
        data2 = format(fed_tax[i], currency) + tab + format(state_tax[i], currency) + tab
        data3 = format(soc_sec[i], currency) + tab + format(medicare[i], currency) + tab
        data4 = format(ret401k[i], currency) + tab + format(net_pay[i], currency)
